...
// Fork of golang upstream time/rate/rate.go
// This fork adds k8s.io/utils/clock as the source of time.
...
func (lim *Limiter) Tokens() float64 {
	return lim.TokensAt(lim.clock.Now())
}

// NewLimiter returns a new Limiter that allows events up to rate r and permits
// bursts of at most b tokens.
func NewLimiter(r Limit, b int, c clock.WithTicker) *Limiter {
	if c == nil {
...
// Delay is shorthand for DelayFrom(clock.Now()).
...
// Cancel is shorthand for CancelAt(clock.Now()).
...
// Reserve is shorthand for ReserveN(clock.Now(), 1).
...
func (lim *Limiter) WaitN(ctx context.Context, n int) (err error) {
	...
	newTimer := func(d time.Duration) (<-chan time.Time, func() bool, func()) {
		timer := lim.clock.NewTimer(d)
		return timer.C(), timer.Stop, func() {}
	}

	return lim.wait(ctx, n, lim.clock.Now(), newTimer)
}
...
// SetLimit is shorthand for SetLimitAt(clock.Now(), newLimit).
...
// SetBurst is shorthand for SetBurstAt(clock.Now(), newBurst).
func (lim *Limiter) SetBurst(newBurst int) {
	lim.SetBurstAt(lim.clock.Now(), newBurst)
}
